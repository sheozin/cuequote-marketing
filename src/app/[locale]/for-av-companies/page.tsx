...
export default async function ForAvCompaniesPage() {
  ...
  const featureWide = { icon: <RefreshCw size={22} />, titleKey: "feat7Title" as const, descKey: "feat7Desc" as const };
  ...
  return (
    <>
      ...
      <section style={{ padding: "100px 24px", background: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", gap: 80 }} className="av-showcase-row">
          <div style={{ flex: "1 1 360px", position: "relative", zIndex: 1 }}>
            <div style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: "#059669", textTransform: "uppercase", letterSpacing: 1.5 }}>{t("showcaseProposalBadge", { defaultValue: "AI PROPOSALS" })}</span>
            </div>
            <h2 style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 800, fontSize: 32, color: "#08172E", lineHeight: 1.15, marginBottom: 20 }}>
              {t("showcaseProposalTitle", { defaultValue: "From event brief to branded proposal in 2 minutes" })}
            </h2>
            <p style={{ fontSize: 16, color: "#6b7280", lineHeight: 1.7, marginBottom: 28 }}>
              {t("showcaseProposalDesc", { defaultValue: "Describe the event, AI generates a complete equipment list from your catalog — with your prices, your terms, ready to send." })}
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                t("showcaseProposalBullet1", { defaultValue: "AI generates from your exact catalog prices" }),
                t("showcaseProposalBullet2", { defaultValue: "Track sent, viewed, won, and lost" }),
                t("showcaseProposalBullet3", { defaultValue: "E-signatures and client portal built in" }),
              ].map(item => (
          ...
          <div style={{ flex: "1 1 520px", position: "relative" }}>
            ...
            <img src="/screenshots/12-proposal-editor.png" alt="CueQuote proposal editor with AI-generated line items" style={{
        ...
        <style>{`
          @media (max-width: 768px) {
            .av-showcase-row { flex-direction: column !important; gap: 40px !important; }
          }
        `}</style>
      </section>

      {/* ── Screenshot: Equipment Catalog ─────────────────────────────────── */}
      <section style={{ padding: "100px 24px", background: "#f9fafb", position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", gap: 80 }} className="av-showcase-row-reverse">
          <div style={{ flex: "1 1 520px", position: "relative", order: 1 }}>
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/screenshots/03-catalog.png" alt="Equipment catalog with categories and pricing" style={{
              width: "100%", borderRadius: 16, position: "relative", zIndex: 1,
              boxShadow: "0 25px 60px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.06)",
            }} />
          </div>
          <div style={{ flex: "1 1 360px", position: "relative", zIndex: 1, order: 2 }}>
            <div style={{
              display: "inline-flex", alignItems: "center", gap: 8,
              background: "#eff6ff", borderRadius: 20, padding: "6px 14px", marginBottom: 20,
            }}>
              <Package size={14} style={{ color: "#3b82f6" }} />
              <span style={{ fontSize: 12, fontWeight: 700, color: "#2563eb", textTransform: "uppercase", letterSpacing: 1.5 }}>{t("showcaseCatalogBadge", { defaultValue: "EQUIPMENT CATALOG" })}</span>
            </div>
            <h2 style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 800, fontSize: 32, color: "#08172E", lineHeight: 1.15, marginBottom: 20 }}>
              {t("showcaseCatalogTitle", { defaultValue: "Your equipment. Your prices. Always accurate." })}
            </h2>
            <p style={{ fontSize: 16, color: "#6b7280", lineHeight: 1.7, marginBottom: 28 }}>
              {t("showcaseCatalogDesc", { defaultValue: "Import your equipment list or build it in CueQuote. The AI pulls from your catalog for every proposal — exact prices, never estimates." })}
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                t("showcaseCatalogBullet1", { defaultValue: "Price Sync keeps catalog and proposals aligned" }),
                t("showcaseCatalogBullet2", { defaultValue: "Duplicate items with one click" }),
                t("showcaseCatalogBullet3", { defaultValue: "AI suggests adding new items as you go" }),
              ].map(item => (
                <div key={item} style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <style>{`
          @media (max-width: 768px) {
            .av-showcase-row-reverse { flex-direction: column !important; gap: 40px !important; }
            .av-showcase-row-reverse > div:first-child { order: 2 !important; }
            .av-showcase-row-reverse > div:last-child { order: 1 !important; }
          }
        `}</style>
      </section>
      ...
      <section id="features" style={{ padding: "96px 24px", background: "#fff" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
            {featuresGrid.map(({ icon, titleKey, descKey }) => (
              <div key={titleKey} style={{
          ...
          {/* Full-width card — avoids orphan */}
          <div style={{
            marginTop: 24, background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 16, padding: "28px 32px",
            display: "flex", alignItems: "center", gap: 20, transition: "box-shadow 0.3s, transform 0.3s",
          }} className="av-feature-card">
            ...
            <div>
              <h3 style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 700, fontSize: 18, color: "#08172E", marginBottom: 4 }}>{t(featureWide.titleKey)}</h3>
              <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.6 }}>{t(featureWide.descKey)}</p>
            </div>
          </div>
        </div>
        <style>{`
          .av-feature-card:hover { box-shadow: 0 8px 24px rgba(0,0,0,0.08); transform: translateY(-2px); }
        `}</style>
      </section>
